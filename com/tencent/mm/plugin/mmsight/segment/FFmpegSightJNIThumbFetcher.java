...
public class FFmpegSightJNIThumbFetcher implements e {
    ...
    public void init(String str, int i, int i2, int i3) {
        w.i(TAG, "init() called with: path = [" + str + "], segment_interval = [" + i + "], width = [" + i2 + "], height = [" + i3 + "]");
        ...
        w.i(TAG, "FFmpegSightJNIThumbFetcher.init. scaled size is (%d, %d); raw size is (%d, %d)", new Object[]{Integer.valueOf(this.mScaledWidth), Integer.valueOf(this.mScaledHeight), Integer.valueOf(videoWidth), Integer.valueOf(videoHeight)});
    }
    ...
    public void reuseBitmap(Bitmap bitmap) {
        w.i(TAG, "reuseBitmap() called with: bitmap = [" + bitmap + "]");
        if (bitmap != null) {
            this.mReusedBitmap = bitmap;
        }
    }
    ...
    public Bitmap getFrameAtTime(long j) {
        long j2;
        long j3 = 0;
        w.i(TAG, "getFrameAtTime() called with: timeMs = [%d], mBufId = [%d]", new Object[]{Long.valueOf(j), Integer.valueOf(this.mBufId)});
        ...
        w.i(TAG, "getFrameAtTime() seekStream return %d", new Object[]{Integer.valueOf(seekStreamWithFlag)});
        if (this.mReusedBitmap == null || this.mReusedBitmap.isRecycled() || this.mReusedBitmap.getWidth() != this.mScaledWidth || this.mReusedBitmap.getHeight() != this.mScaledHeight) {
            ...
            this.mReusedBitmap = o.hgW.a(new b(this.mScaledWidth, this.mScaledHeight));
        }
        seekStreamWithFlag = SightVideoJNI.drawScaledFrame(this.mBufId, this.mReusedBitmap, this.mScaledWidth, this.mScaledHeight);
        w.i(TAG, "getFrameAtTime() dr return %d", new Object[]{Integer.valueOf(seekStreamWithFlag)});
    ...
    public int getDurationMs() {
        w.i(TAG, "getDurationMs() returned: " + this.mDurationMs);
        return this.mDurationMs;
    }

    public int getScaledWidth() {
        w.i(TAG, "getScaledWidth() returned: " + this.mScaledWidth);
        return this.mScaledWidth;
    }

    public int getScaledHeight() {
        w.i(TAG, "getScaledHeight() returned: " + this.mScaledHeight);
        return this.mScaledHeight;
    }

    public void release() {
        w.i(TAG, "release() called");
