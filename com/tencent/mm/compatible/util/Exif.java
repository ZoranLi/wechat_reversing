...
public class Exif {
    ...
    public int parseFromFile(String str) {
        if (new File(str).exists()) {
            w.i(TAG, "parse EXIF from file on real fs, result: %d, path: %s", Integer.valueOf(nativeParseFile(str)), str);
    ...
    public int parseFromStream(InputStream inputStream) {
        ...
        w.i(TAG, "parse EXIF from Stream, ret = " + nativeParse);
        return nativeParse;
    }
    ...
    public static Exif fromFile(String str) {
        Exif exif = new Exif();
        try {
            w.i(TAG, "parse EXIF, result: " + exif.parseFromFile(str));
        } catch (IOException e) {
            w.w(TAG, "parse EXIF failed: " + e.getMessage());
        }
        return exif;
    }
}
