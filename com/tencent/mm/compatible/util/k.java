...
public final class k {
    ...
    private static HashMap<String, String> gSZ = null;
    ...
    public static String dy(String str) {
        ...
        File file = new File(ab.getContext().getDir("recover_lib", 0), "lib" + str + ".so");
        if (file.canRead()) {
    ...
    private static boolean a(String str, ClassLoader classLoader) {
        ...
        if (!gSY) {
            ...
            HashMap as = (tinkerResultIntent == null || ShareIntentUtil.an(tinkerResultIntent) != 0) ? null : ShareIntentUtil.as(tinkerResultIntent);
            gSZ = as;
            c = b.c(tinkerApplicationLike);
            if (ShareTinkerInternals.mA(c)) {
                gSX = null;
            } else {
                gSX = new File(SharePatchFileUtil.hx(tinkerApplicationLike.getApplication()).getAbsolutePath() + "/" + SharePatchFileUtil.VS(c)).getAbsolutePath() + "/lib";
            }
        }
        ...
        if (ShareTinkerInternals.DX(tinkerApplicationLike.getTinkerFlags()) && b.b(tinkerApplicationLike) && gSZ != null) {
            for (String c2 : gSZ.keySet()) {
                if (c2.equals(str2)) {
                    String str3 = gSX + "/" + c2;
                    File file = new File(str3);
                    if (!file.exists()) {
                        continue;
                    } else if (!tinkerApplicationLike.getTinkerLoadVerifyFlag() || SharePatchFileUtil.e(file, (String) gSZ.get(c2))) {
                        try {
                            w.w("MicroMsg.LoadLibrary", "succ load from patch path: %s", str3);
                        ...
                        w.e("MicroMsg.LoadLibrary", "loadLibraryFromTinker md5mismatch fail: %s", str3);
                    }
                }
            }
        }
    ...
    public static void si() {
        ...
        File dir = ab.getContext().getDir("recover_lib", 0);
        ZipFile zipFile = new ZipFile(ab.getContext().getApplicationInfo().sourceDir);
        try {
