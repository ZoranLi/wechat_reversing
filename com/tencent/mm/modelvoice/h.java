...
public final class h implements b {
    ...
    public final void Lu() {
        if (this.ibc != null) {
            try {
                this.ibc.close();
                this.ibc = null;
                w.d("MicroMsg.SilkFileOperator", "Close :" + this.fyF);
            } catch (IOException e) {
            }
        }
    }
    ...
    private boolean lO(String str) {
        ...
        Assert.assertTrue(z);
        w.d("MicroMsg.SilkFileOperator", "Open file:" + this.ibc + " mode:" + str);
        try {
            this.ibc = new RandomAccessFile(this.fyF, str);
            return true;
        } catch (Exception e) {
            w.e("MicroMsg.SilkFileOperator", "ERR: OpenFile[" + this.fyF + "] failed:[" + e.getMessage() + "]");
            this.ibc = null;
            return false;
        }
    }
    ...
    public final g bd(int i, int i2) {
        ...
        if (i < 0 || i2 <= 0) {
            ...
            try {
                ...
                w.d("MicroMsg.SilkFileOperator", "DBG: ReadFile[" + this.fyF + "] readOffset:" + i + " readRet:" + read + " fileNow:" + this.ibc.getFilePointer() + " fileSize:" + length);
                if (read < 0) {
                    read = 0;
                }
                gVar.fAK = read;
                ...
                w.e("MicroMsg.SilkFileOperator", "ERR: ReadFile[" + this.fyF + "] Offset:" + i + "  failed:[" + e.getMessage() + "] ");
                Lu();
                gVar.ret = -1;
            }
        } else {
    ...
    public final int write(byte[] bArr, int i, int i2) {
        ...
        try {
            ...
            w.e("MicroMsg.SilkFileOperator", "ERR: WriteFile[" + this.fyF + "] Offset:" + i2 + " failed:[" + e.getMessage() + "]");
            Lu();
            return -3;
        }
    }
