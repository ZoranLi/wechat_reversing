package com.tencent.smtt.utils;

import android.annotation.SuppressLint;
import android.content.Context;
import android.os.Looper;
import android.view.View;
import android.view.ViewGroup;
import android.widget.FrameLayout;
import android.widget.FrameLayout.LayoutParams;
import android.widget.RelativeLayout;
import android.widget.TextView;
import android.widget.Toast;
import com.samsung.android.sdk.look.smartclip.SlookSmartClipMetaTag;
import com.tencent.smtt.sdk.WebView;
import com.tencent.tmassistantsdk.downloadservice.Downloads;
import dalvik.system.DexClassLoader;
import java.io.File;
import java.io.InputStream;
import java.io.OutputStream;
import java.net.HttpURLConnection;
import java.net.URL;
import java.util.HashMap;
import java.util.Map;

public final class e {
    private static DexClassLoader wXF = null;
    public static Looper wXG = null;
    private static e wXH = null;
    public String wXE = "";

    public interface a {
        void cfh();

        void cfi();

        void iw(int i);
    }

    private e(Context context) {
        this.wXE = context.getDir("debugtbs", 0).getAbsolutePath() + File.separator + "plugin";
    }

    public static e gS(Context context) {
        if (wXH == null) {
            wXH = new e(context);
        }
        return wXH;
    }

    @SuppressLint({"NewApi"})
    public final void a(String str, WebView webView, Context context, Looper looper) {
        TbsLog.i("debugtbs", "showPluginView -- url: " + str + "; webview: " + webView + "; context: " + context);
        String str2 = this.wXE + File.separator + "DebugPlugin.tbs";
        String str3 = this.wXE + File.separator + "DebugPlugin.apk";
        File file = new File(str2);
        File file2 = new File(str3);
        wXG = looper;
        if (file.exists()) {
            file2.delete();
            file.renameTo(file2);
        }
        if (file2.exists()) {
            try {
                str2 = c.d(context, new File(str3));
                if ("".equals(str2)) {
                    str2 = this.wXE + File.separator + "opt";
                    file = new File(str2);
                    if (!file.exists()) {
                        file.mkdirs();
                    }
                    if (wXF == null) {
                        wXF = new DexClassLoader(str3, str2, null, context.getClassLoader());
                    }
                    Map hashMap = new HashMap();
                    hashMap.put(SlookSmartClipMetaTag.TAG_TYPE_URL, str);
                    hashMap.put("tbs_version", WebView.getTbsSDKVersion(context));
                    hashMap.put("tbs_core_version", WebView.getTbsCoreVersion(context));
                    if (wXG != null) {
                        hashMap.put("looper", looper);
                    }
                    Object newInstance = wXF.loadClass("com.tencent.tbs.debug.plugin.DebugView").getConstructor(new Class[]{Context.class, Map.class}).newInstance(new Object[]{context, hashMap});
                    if (newInstance instanceof FrameLayout) {
                        FrameLayout frameLayout = (FrameLayout) newInstance;
                        webView.addView(frameLayout, new LayoutParams(-1, -1));
                        TbsLog.i("debugtbs", "show " + frameLayout + " successful in " + webView);
                        return;
                    }
                    TbsLog.e("debugtbs", "get debugview failure: " + newInstance);
                    return;
                }
                TbsLog.e("debugtbs", "verifyPlugin apk: " + str3 + " signature failed - sig: " + str2);
                Toast.makeText(context, "插件校验失败，请重试", 0).show();
                file.delete();
                file2.delete();
                return;
            } catch (Exception e) {
                f.J(file2);
                return;
            }
        }
        TbsLog.i("debugtbs", "showPluginView - going to download plugin...");
        final View relativeLayout = new RelativeLayout(context);
        final View textView = new TextView(context);
        ViewGroup.LayoutParams layoutParams = new RelativeLayout.LayoutParams(-2, -2);
        layoutParams.addRule(13);
        textView.setText("加载中，请稍后...");
        relativeLayout.addView(textView, layoutParams);
        webView.addView(relativeLayout, new LayoutParams(-1, -1));
        final String str4 = this.wXE + File.separator + "DebugPlugin.tbs";
        f.J(new File(str4));
        final WebView webView2 = webView;
        final Context context2 = context;
        final String str5 = str;
        final a anonymousClass1 = new a(this) {
            final /* synthetic */ e wXL;

            public final void cfh() {
                webView2.post(new Runnable(this) {
                    final /* synthetic */ AnonymousClass1 wXM;

                    {
                        this.wXM = r1;
                    }

                    public final void run() {
                        Toast.makeText(context2, "下载成功", 0).show();
                        relativeLayout.setVisibility(4);
                        this.wXM.wXL.a(str5, webView2, context2, e.wXG);
                    }
                });
            }

            public final void cfi() {
                webView2.post(new Runnable(this) {
                    final /* synthetic */ AnonymousClass1 wXM;

                    {
                        this.wXM = r1;
                    }

                    public final void run() {
                        Toast.makeText(context2, "下载失败，请检查网络", 0).show();
                    }
                });
            }

            public final void iw(final int i) {
                webView2.post(new Runnable(this) {
                    final /* synthetic */ AnonymousClass1 wXM;

                    public final void run() {
                        textView.setText("已下载" + i + "%");
                    }
                });
            }
        };
        new Thread() {
            public final void run() {
                InputStream inputStream;
                OutputStream L;
                InputStream inputStream2;
                Throwable th;
                Throwable th2;
                OutputStream outputStream = null;
                try {
                    HttpURLConnection httpURLConnection = (HttpURLConnection) new URL("http://soft.tbs.imtt.qq.com/17421/tbs_res_imtt_tbs_DebugPlugin_DebugPlugin.tbs").openConnection();
                    int contentLength = httpURLConnection.getContentLength();
                    httpURLConnection.setConnectTimeout(Downloads.MIN_RETYR_AFTER);
                    httpURLConnection.connect();
                    inputStream = httpURLConnection.getInputStream();
                    try {
                        L = f.L(new File(str4));
                        try {
                            byte[] bArr = new byte[8192];
                            int i = 0;
                            while (true) {
                                int read = inputStream.read(bArr);
                                if (read <= 0) {
                                    break;
                                }
                                i += read;
                                L.write(bArr, 0, read);
                                anonymousClass1.iw((i * 100) / contentLength);
                            }
                            anonymousClass1.cfh();
                            try {
                                inputStream.close();
                            } catch (Exception e) {
                            }
                            try {
                                L.close();
                            } catch (Exception e2) {
                            }
                        } catch (Exception e3) {
                            inputStream2 = inputStream;
                            try {
                                anonymousClass1.cfi();
                                try {
                                    inputStream2.close();
                                } catch (Exception e4) {
                                }
                                try {
                                    L.close();
                                } catch (Exception e5) {
                                }
                            } catch (Throwable th3) {
                                th = th3;
                                inputStream = inputStream2;
                                outputStream = L;
                                th2 = th;
                                try {
                                    inputStream.close();
                                } catch (Exception e6) {
                                }
                                try {
                                    outputStream.close();
                                } catch (Exception e7) {
                                }
                                throw th2;
                            }
                        } catch (Throwable th4) {
                            th = th4;
                            outputStream = L;
                            th2 = th;
                            inputStream.close();
                            outputStream.close();
                            throw th2;
                        }
                    } catch (Exception e8) {
                        L = null;
                        inputStream2 = inputStream;
                        anonymousClass1.cfi();
                        inputStream2.close();
                        L.close();
                    } catch (Throwable th5) {
                        th2 = th5;
                        inputStream.close();
                        outputStream.close();
                        throw th2;
                    }
                } catch (Exception e9) {
                    L = null;
                    anonymousClass1.cfi();
                    inputStream2.close();
                    L.close();
                } catch (Throwable th6) {
                    th2 = th6;
                    inputStream = null;
                    inputStream.close();
                    outputStream.close();
                    throw th2;
                }
            }
        }.start();
    }
}
